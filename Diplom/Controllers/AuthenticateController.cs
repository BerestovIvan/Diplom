...
namespace WebForumMVC.Controllers
{
    ...
    {
        readonly IAuthorizationServiceBll authorizationService;
        readonly IMapper mapper;
        public AuthenticateController(IAuthorizationServiceBll authorizationService, IMapper mapper)
        {
        ...
        public async Task<IActionResult> Register(RegisterPostModel registerPostModel)
        {
            ...
            var result = await authorizationService.Register(mapper.Map<RegisterModel>(registerPostModel));
            if (result == null)
            {
        ...
        public async Task<IActionResult> RegisterAdmin(RegisterPostModel registerPostModel)
        {
            ...
            var result = await authorizationService.RegisterAdmin(mapper.Map<RegisterModel>(registerPostModel));
            if (result == null)
            {
                return StatusCode(StatusCodes.Status500InternalServerError,
        ...
        public async Task<IActionResult> Login(LoginPostModel model)
        {
            ...
            var loginResultModel = await authorizationService.Login(mapper.Map<LoginModel>(model));

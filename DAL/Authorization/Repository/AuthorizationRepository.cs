...
namespace DAL.Authorization.Repository
{
    public class AuthorizationRepository : IAuthorizationRepository
    {
        ...
        public async Task<IdentityResult> Register(ApplicationUser applicationUser)
        {
            ...
            if (!result.Succeeded)
                return result;
            if (await roleManager.RoleExistsAsync(UserRoles.Buyer.ToString()))
            {
                await userManager.AddToRoleAsync(applicationUser, UserRoles.Buyer.ToString());
            }
            return result;
        }
        ...
        public async Task<IdentityResult> RegisterAdmin(ApplicationUser applicationUser)
        {
            ...
            if (!await roleManager.RoleExistsAsync(UserRoles.Buyer.ToString()))
                await roleManager.CreateAsync(new IdentityRole(UserRoles.Buyer.ToString()));
            if (await roleManager.RoleExistsAsync(UserRoles.Admin.ToString()))
            {
